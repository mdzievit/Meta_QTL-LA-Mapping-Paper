...
```{r Packages and Session Info, warning=FALSE}
library(tidyverse)
library(broom)
library(grid)
library(gridExtra)
...
```{r Process F2 Data, warning=FALSE}
...
b73_test <- t.test(x = data_formatted %>% filter(Population == "B73/PHW30") %>% pull(LA) %>% na.omit(),
       ...
       var.equal = FALSE)
##Test for B73 reciprocal, difference and then p-value reported
...
mo17_test <- t.test(x = data_formatted %>% filter(Population == "Mo17/PHW30") %>% pull(LA) %>% na.omit(),
       ...
       var.equal = FALSE)
##Test for Mo17 reciprocal, difference and then p-value reported
...
```{r Process F2:3 Data, warning=FALSE}
##########F3 Summary###########
...
```{r Supplemental Figure 1, fig.height=4, fig.width=7, warning=FALSE}
